...
class ConfigMissingListener(PersistentLogger):
    ...
    def run(self, config_field: Config, camera: Camera, **kwargs):
        msg = f'Config {config_field.name} is missing on {camera.name}'
