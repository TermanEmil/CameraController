...
class ConfigFailedChangeListener(PersistentLogger):
    log_type = LogType.ERROR
    category = 'ScheduledConfig'
    title = 'Failed to change config'
    ...
    def run(self, config_field: Config, camera: Camera, current_config_value: str, **kwargs):
        msg = f'Failed to change {config_field.name} to {config_field.value} on {camera.name}. It remained {current_config_value}'
